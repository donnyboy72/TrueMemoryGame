 ...
 */
export const classifySession = (playerMetrics) => {
    ...
    const distances = {
        well_rested: Math.sqrt(Math.pow(normalizedPlayer.accuracy - normalizedReferences.well_rested.accuracy, 2) +
        ...
        somewhat_rested: Math.sqrt(Math.pow(normalizedPlayer.accuracy - normalizedReferences.somewhat_rested.accuracy, 2) +
        ...
        severely_tired: Math.sqrt(Math.pow(normalizedPlayer.accuracy - normalizedReferences.severely_tired.accuracy, 2) +
    ...
    const closestGroup = Object.keys(distances).reduce((a, b) => distances[a] < distances[b] ? a : b);
