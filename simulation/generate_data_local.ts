...
// --- Type Definitions (copied from src/lib/DataLogger.ts) ---
type TrialData = {
  trial_number: number;
...
type CognitiveState = {
  ...
  rtMean: number;
  rtStdDev: number;
  accuracyMean: number; // 0.0 to 1.0
  accuracyStdDev: number;
  spatialErrorMean: number; // pixels, relative to tile center
...
function transformSession(session: SessionLog): StoredSession {
    ...
    for (let i = 0; i < NUM_SESSIONS_PER_CATEGORY; i++) {
      ...
      for (let trialNum = 1; trialNum <= maxTrials && !gameOver; trialNum++) {
        ...
        for (let k = 0; k < correctOrder.length; k++) {
          ...
          if (!isAccurate) {
            ...
            if (Math.random() < 0.7) { // 70% chance of a near-miss
              ...
              if (row > 0) adjacentTiles.push(expectedTileId - GRID_SIZE_DIM);
              if (row < GRID_SIZE_DIM - 1) adjacentTiles.push(expectedTileId + GRID_SIZE_DIM);
              if (col > 0) adjacentTiles.push(expectedTileId - 1);
              if (col < GRID_SIZE_DIM - 1) adjacentTiles.push(expectedTileId + 1);
              ...
              if (adjacentTiles.length > 0) {
                ...
                if (otherTiles.length > 0) clickedTileId = otherTiles[getRandomInt(0, otherTiles.length - 1)];
              }
            } else { // Random miss
              const allTileIds = Array.from({ length: GRID_SIZE_DIM * GRID_SIZE_DIM }).map((_, id) => id);
              const otherTiles = allTileIds.filter(id => id !== expectedTileId);
        ...
        if (!trialCorrect) {
          ...
          currentSequenceLength = Math.max(
            state.initialSequenceLength, 
            Math.min(
              GRID_SIZE_DIM * GRID_SIZE_DIM / 2, // Cap at half the grid size
